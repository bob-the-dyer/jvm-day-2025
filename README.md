...
многопоточности - "Обедающие философы". Варианты решений будут включать в себя реализации на виртуальных и на
...
- как применять инженерный подход для ответа на вопросы выше
...
#### Кажется что тест с чтением с диска может быть некорректным, так как файл может поместиться в кеш и реального чтения с диска не будет происходить. Или наоборот все может упиреться в железные ограничения, как быть?
...
__TODO тут нужен треддампы или какое-то иное объяснение с доказательством, например что 1 платформенный поток перегружен
хоть__
...
#### Почему бенчмарки сделаны на JMH, хотя в данном случае больше подходит JLBH?

Я ничего не знал о JLBH до этого вопроса и похоже что JLBH действительно подходит больше как из-за запуска в контексте,
так и из-за более точечного указания какие куски логики нужно померить. С JLBH можно не мерить логику
...
__TODO если будет время интересно написать объединенный тест и посмотреть на результаты просто чтобы сравнить, также
интересно что там такое за Affinity__
