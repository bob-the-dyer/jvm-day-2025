...
public class LoopingPhilosophersBenchmark {
    ...
    static Runnable eatingByLooping = () -> {
        long startTimeInNanos = System.nanoTime();
        while (System.nanoTime() < startTimeInNanos + 16_000) {
            //TODO что сделать чтобы тут не было оптимизаций?
        }
    };
    ...
    static {
        ...
        for (int i = 0; i < TEST_PHILOSOPHERS_COUNT; i++) {
            ...
            reentrantPhilosophers.add(new ReentrantPhilosopher(i, leftChopstick, rightChopstick, barrier, eatingByLooping));
        }
        for (int i = 0; i < TEST_PHILOSOPHERS_COUNT; i++) {
            Chopstick leftChopstick = chopsticks.get(i);
            Chopstick rightChopstick = chopsticks.get(i != 0 ? i - 1 : TEST_PHILOSOPHERS_COUNT - 1);
            synchronizedPhilosophers.add(new SynchronizedPhilosopher(i, leftChopstick, rightChopstick, barrier, eatingByLooping));
        }
    }
